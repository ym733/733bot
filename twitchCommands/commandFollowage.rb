  ...
  "method" => -> (params) {
    ...
    unless parameters.nil?
      if parameters.length == 1
        ...
        return "only 2 parameters or less needed, usage: followage (user) (channel)"
      end
    else
      if target_channel.nil?
    ...
    followed_at = JSON.parse(HTTPS.get api_url)["followedAt"]

    if followed_at.nil?
      return "#{target_user} does not follow #{target_channel}"
    end

    "#{target_user} has been following #{target_channel} for #{format_time_diff((Time.now - Time.parse(followed_at)))}"
  }
}
